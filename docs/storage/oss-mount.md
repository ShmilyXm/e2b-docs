...
E2B 可以按 Template 配置阿里云 OSS 挂载。配置完成后，使用该 Template **新创建** 的每个 Sandbox 都会在指定目录看到同一份 OSS 数据，适合共享数据集、模型、构建产物和任务结果。
...
| OSS 目录 | 否 | Bucket 内的 Prefix。留空表示 Bucket 根目录；填写时不需要开头或结尾的 `/`。 |
| Sandbox 目录 | 是 | Sandbox 内的绝对路径，例如 `/workspace/data`；不能使用根目录 `/`。 |
| 访问模式 | 是 | `Read only`（只读）或 `Read and write`（读写）。 |

目录路径中不能包含连续的 `//`，也不能包含 `.` 或 `..` 路径段。

## 在 Sandbox 中验证

挂载配置保存后，必须新建 Sandbox 才能验证。下面以挂载到 `/workspace/data` 为例：
...
如果使用读写模式，可以创建测试文件并到 OSS 控制台确认对象：
...
3. Sandbox 目录是否与页面配置完全一致。
4. 挂载是否仍显示在 **Storage** 页面。
